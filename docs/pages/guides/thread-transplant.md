...
Transplant preserves the original thread id, filename, and relative
`sessions/...` path so the target Codex home can resume the same thread id from
disk. By default it rewrites `session_meta.payload.cwd` to the destination
project cwd so the imported thread appears in Codex App for that project.
...
source home, installs it to the same
`sessions/<YYYY>/<MM>/<DD>/<rollout-file>.jsonl` path under the target home,
rewrites the metadata cwd unless `--preserve-cwd` is passed, and verifies the
installed file.

For an exact archival copy that keeps the source cwd and checksum, pass
`--preserve-cwd`:

```bash
codex-toys threads transplant <thread-id> --from-codex-home <source> --to-codex-home <target> --preserve-cwd
```

To replace an existing rollout, opt in explicitly:

```bash
codex-toys threads transplant <thread-id> --from-codex-home <source> --to-codex-home <target> --cwd <project> --replace
```
...
path from the rollout filename timestamp. The installed file rewrites
`session_meta.payload.cwd` to the destination project cwd unless
`--preserve-cwd` is passed. Existing target files fail by default; pass
`--replace` to write a timestamped backup and replace them.
...
preserves the original thread id by installing the native rollout into the
target home, then using the existing Codex/app-server thread resume or fork
behavior.
