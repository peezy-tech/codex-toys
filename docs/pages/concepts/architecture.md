...
Workbench schedules and operator requests can also create deferred run intents,
which the toybox claims and executes through the same turn automation,
workbench task, or native turn primitives.
...
pass-through, workbench functions, delegation, automation helpers, and the
...
`/api/schema`, and call generic app/workbench methods. The proxy does not add a
