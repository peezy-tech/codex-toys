...
A codex-flows workspace agent is the logical runtime behind an operator surface
...
`account/read` are forwarded through `appServer.call`.
...
Use an agent when a product needs Codex workspace control locally or over SSH.
