...
const resultPrefix = "WORKFLOW_MODULE_RESULT ";
...
try {
	if (!scriptPath) {
		throw new Error("Workflow module runner requires a script path");
	}
	const context = await readContext();
	const moduleUrl = pathToFileURL(path.resolve(scriptPath));
	moduleUrl.searchParams.set("workflowRun", `${Date.now()}`);
	const module = await import(moduleUrl.href) as { default?: unknown };
	if (typeof module.default !== "function") {
		throw new Error("Workflow module must export a default handler function");
	}
	const result = await module.default(scriptContext(context)) as unknown;
	if (!isRecord(result)) {
		throw new Error("Workflow module must return a JSON object");
	}
...
function scriptContext(context: unknown): unknown {
	...
	return {
		...
		workbench: {
			call: async (method: string, params?: unknown) =>
				await callHost("workbench.call", { method, params }),
		},
		turn: {
...
function callHost(method: string, params?: unknown): Promise<unknown> {
	if (typeof process.send !== "function") {
		return Promise.reject(new Error("Workflow host API requires IPC"));
	}
...
async function readContext(): Promise<unknown> {
	if (typeof process.send !== "function") {
		throw new Error("Workflow module runner requires IPC");
	}
...
function isContextMessage(value: unknown): value is {
	...
	return message.type === "workflow.context";
}
...
function isHostResponseMessage(value: unknown): value is {
	...
	return message.type === "workflow.hostResponse" &&
