...
export class CodexStdioTransport extends CodexEventEmitter {
  ...
  start(): void {
    ...
    child.once("exit", (code, signal) => {
      ...
      const exitError = new Error(`codex app-server exited with ${code ?? signal ?? "unknown"}`);
    ...
    void readLines(child.stderr, (line) => this.emit("stderr", line)).catch((error) =>
      this.emit("error", error),
    );
  }
  ...
  #handleLine(line: string): void {
    ...
    try {
      ...
      this.emit("error", new Error(`Failed to parse app-server JSON-RPC line: ${String(error)}`));
      return;
    }
...
function killChildProcessGroup(child: AppServerProcess, signal: NodeJS.Signals): void {
...
function defaultCodexArgs(appServerSocket: string | undefined): string[] {
  ...
  return ["app-server", "--listen", "stdio://", "--enable", "apps"];
}
