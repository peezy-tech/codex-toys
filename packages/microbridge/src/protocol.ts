...
export const MICROBRIDGE_INITIALIZE_METHOD = "microbridge.initialize";
export const MICROBRIDGE_EVENT_METHOD = "microbridge.event";
...
export type MicrobridgeInitializeParams = {
	clientInfo?: {
		name?: string;
		title?: string | null;
		version?: string;
...
export type MicrobridgeInitializeResponse = {
	ok: true;
	serverInfo: {
		name: string;
		version: string;
...
export type MicrobridgeMethodMetadata = {
	name: string;
	description?: string;
...
export type MicrobridgeEventParams = {
	event: MicrobridgeEvent;
};

export function appCallParams(value: unknown): AppCallParams | undefined {
	const input = record(value);
	const method = stringValue(input.method);
	return method ? { method, params: input.params } : undefined;
}

export function appNotifyParams(value: unknown): AppNotifyParams | undefined {
	const input = record(value);
	const method = stringValue(input.method);
	return method ? { method, params: input.params } : undefined;
}

export function appRespondParams(value: unknown): AppRespondParams | undefined {
	const input = record(value);
...
export function appRespondErrorParams(value: unknown): AppRespondErrorParams | undefined {
	const input = record(value);
	const id = jsonRpcIdValue(input.id);
...
export function appNotificationParams(value: unknown): AppNotificationParams | undefined {
	const input = record(value);
	const message = jsonRpcNotification(input.message);
	return message ? { message } : undefined;
}

export function appRequestParams(value: unknown): AppRequestParams | undefined {
	const input = record(value);
	const message = jsonRpcRequest(input.message);
	return message ? { message } : undefined;
}

export function microbridgeEventParams(value: unknown): MicrobridgeEventParams | undefined {
	const input = record(value);
	const event = record(input.event);
	const type = stringValue(event.type);
	return type ? { event: event as unknown as MicrobridgeEvent } : undefined;
}
