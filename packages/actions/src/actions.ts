...
const ACTIONS_STATE_RELATIVE = path.join("workbench", "actions");

export function repoCodexHome(workbenchRoot: string): string {
	return path.join(path.resolve(workbenchRoot), ".codex");
}
...
export async function prepareActionsCodexAuth(
	...
	const codexHome = repoCodexHome(options.workbenchRoot);
...
export async function cleanupActionsCodexHome(
	...
	for (const relativePath of [
		...
		path.join("memories", "phase2_workbench_diff.md"),
	]) {
