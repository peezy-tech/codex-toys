...
export type SanitizeWorkspaceMemoryArtifactsOptions = {
	workspaceRoot: string;
};

export type SanitizeWorkspaceMemoryArtifactsResult = {
	memoryRoot: string;
	removed: string[];
};
...
export async function copyCodexMemoryArtifacts(
	...
	const destinationCodexHome = path.join(path.resolve(options.workspaceRoot), ".codex");
...
export async function sanitizeWorkspaceMemoryArtifacts(
	options: SanitizeWorkspaceMemoryArtifactsOptions,
): Promise<SanitizeWorkspaceMemoryArtifactsResult> {
	const memoryRoot = path.join(path.resolve(options.workspaceRoot), ".codex", "memories");
	const removed: string[] = [];
	await removeIfExists(path.join(memoryRoot, ".git"), memoryRoot, removed);
	await removeIfExists(path.join(memoryRoot, "phase2_workspace_diff.md"), memoryRoot, removed);
