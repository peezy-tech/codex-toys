...
import { createCodexFlowsProxyHandler } from "../proxy.ts";
...
server.listen(parsed.port, parsed.host, () => {
	...
	process.stderr.write(`codex-flows-proxy listening on http://${parsed.host}:${port}\n`);
});
...
function parseArgs(argv: string[]): ParsedProxyArgs {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		if (arg === "--agent-command") {
			result.agentCommand = required(argv, ++index, arg);
			continue;
		}
		if (arg.startsWith("--agent-command=")) {
			result.agentCommand = arg.slice("--agent-command=".length);
			continue;
		}
...
function helpText(): string {
	return `codex-flows-proxy serves a generic HTTP edge for codex-flows agents.
...
Options:
  --cwd <path>                 Workspace cwd for the local or SSH agent.
  --ssh, --ssh-target <target> SSH target for remote agent stdio.
  ...
  --timeout-ms <ms>            Agent request timeout. Defaults to 90000.
  --agent-command <command>    codex-flows command/path for spawned agents.
  --codex-command <command>    Codex command used inside the agent.
