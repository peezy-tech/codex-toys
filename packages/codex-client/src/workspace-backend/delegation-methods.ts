...
import type { WorkspaceBackendMethodHandler } from "./server.ts";
import type { WorkspaceMethodMetadata } from "./protocol.ts";
import {
...
export const WORKSPACE_DELEGATION_LIST_METHOD = "delegation.list";
export const WORKSPACE_DELEGATION_START_METHOD = "delegation.start";
export const WORKSPACE_DELEGATION_RESUME_METHOD = "delegation.resume";
export const WORKSPACE_DELEGATION_SEND_METHOD = "delegation.send";
export const WORKSPACE_DELEGATION_READ_METHOD = "delegation.read";
export const WORKSPACE_DELEGATION_SET_POLICY_METHOD = "delegation.setPolicy";
export const WORKSPACE_DELEGATION_FLUSH_RESULTS_METHOD = "delegation.flushResults";
export const WORKSPACE_DELEGATION_LIST_GROUPS_METHOD = "delegation.listGroups";

export const workspaceDelegationMethodMetadata: WorkspaceMethodMetadata[] = [
	{
		name: WORKSPACE_DELEGATION_LIST_METHOD,
		description: "List delegated Codex threads recorded for this workspace.",
		sideEffects: "read-only",
		category: "delegation",
	},
	{
		name: WORKSPACE_DELEGATION_START_METHOD,
		description: "Start a delegated Codex thread in a workspace-relative cwd.",
		sideEffects: "writes-local",
		category: "delegation",
	},
	{
		name: WORKSPACE_DELEGATION_RESUME_METHOD,
		description: "Resume an existing delegated thread.",
		sideEffects: "writes-local",
		category: "delegation",
	},
	{
		name: WORKSPACE_DELEGATION_SEND_METHOD,
		description: "Send a follow-up turn to a delegated thread.",
		sideEffects: "writes-local",
		category: "delegation",
	},
	{
		name: WORKSPACE_DELEGATION_READ_METHOD,
		description: "Read the latest state for a delegated thread.",
		sideEffects: "writes-local",
		category: "delegation",
	},
	{
		name: WORKSPACE_DELEGATION_SET_POLICY_METHOD,
		description: "Set return policy metadata for a delegation.",
		sideEffects: "writes-local",
		category: "delegation",
	},
	{
		name: WORKSPACE_DELEGATION_FLUSH_RESULTS_METHOD,
		description: "Collect completed delegation results for return handling.",
		sideEffects: "writes-local",
		category: "delegation",
	},
	{
		name: WORKSPACE_DELEGATION_LIST_GROUPS_METHOD,
...
export type WorkspaceDelegationTarget = {
	id: string;
	cwd: string;
	label: string;
	kind: "workspace" | "repo" | "recent";
	source: "discovered" | "recent";
	exists: boolean;
};

export type WorkspaceDelegationListResponse = {
	delegations: WorkspaceDelegation[];
	targets?: WorkspaceDelegationTarget[];
};

export type WorkspaceDelegationRuntimeOptions = {
	appServer: {
...
export function createWorkspaceDelegationMethods(
	options: WorkspaceDelegationRuntimeOptions,
): Record<string, WorkspaceBackendMethodHandler> {
	const workspaceRoot = path.resolve(options.workspaceRoot ?? process.cwd());
	const statePath = options.statePath ??
		path.join(workspaceRoot, ".codex", "workspace", "local", "delegations.json");
	const env = options.env ?? process.env;
	const state: WorkspaceDelegationState = { delegations: [] };
	const store = new WorkspaceDelegationStore(statePath);
	const capability = new WorkspaceDelegationCapability({
		...
		turnStartParams: ({ threadId, prompt, cwd, args }) =>
			turnStartParams(threadId, prompt, cwd, args),
		metadataFromArgs: (args) => metadataFromArgs(args, workspaceRoot),
		surfaceKeyForCwd: (cwd) => workspaceKeyForCwd(workspaceRoot, cwd),
	});
	...
	return {
		[WORKSPACE_DELEGATION_LIST_METHOD]: async (params) => {
			await load();
			const args = record(params);
			const response: WorkspaceDelegationListResponse = capability.list();
			if (booleanValue(args.includeTargets, true)) {
				response.targets = await listDelegationTargets(workspaceRoot, state);
			}
			return response;
		},
		[WORKSPACE_DELEGATION_START_METHOD]: async (params) =>
			await mutate(async () => {
		...
		[WORKSPACE_DELEGATION_RESUME_METHOD]: async (params) =>
			await mutate(async () => {
		...
		[WORKSPACE_DELEGATION_SEND_METHOD]: async (params) =>
			await mutate(async () => await capability.send(record(params))),
		[WORKSPACE_DELEGATION_READ_METHOD]: async (params) =>
			await mutate(async () => await capability.read(record(params))),
		[WORKSPACE_DELEGATION_SET_POLICY_METHOD]: async (params) =>
			await mutate(() => capability.setPolicy(record(params))),
		[WORKSPACE_DELEGATION_FLUSH_RESULTS_METHOD]: async (params) =>
			await mutate(async () => await capability.flushResults(record(params))),
		[WORKSPACE_DELEGATION_LIST_GROUPS_METHOD]: async () => {
			await load();
...
export async function resolveDelegationCwd(
	...
	const allowAbsoluteCwd = options.allowAbsoluteCwd === true ||
		truthy(options.env?.CODEX_FLOWS_DELEGATION_ALLOW_ABSOLUTE_CWD);
	let resolved: string;
	if (value === "@") {
		resolved = workspaceRoot;
	} else if (value.startsWith("@/") || value.startsWith("@\\")) {
		resolved = path.resolve(workspaceRoot, value.slice(2));
		assertInsideWorkspace(workspaceRoot, resolved, value);
	} else if (path.isAbsolute(value)) {
		if (!allowAbsoluteCwd) {
			throw new Error(
				"Absolute delegation cwd requires allowAbsoluteCwd=true or CODEX_FLOWS_DELEGATION_ALLOW_ABSOLUTE_CWD=1",
			);
		}
		resolved = path.resolve(value);
	} else {
		resolved = path.resolve(workspaceRoot, value);
		assertInsideWorkspace(workspaceRoot, resolved, value);
	}
...
function threadStartParams(
	...
	return compactUndefined({
		...
		runtimeWorkspaceRoots: stringArray(args.runtimeWorkspaceRoots),
...
async function listDelegationTargets(
...
): Promise<WorkspaceDelegationTarget[]> {
	const targets = new Map<string, WorkspaceDelegationTarget>();
	for (const collection of [
		{ dirname: "workspaces", kind: "workspace" as const },
		{ dirname: "repos", kind: "repo" as const },
	]) {
		const parent = path.join(workspaceRoot, collection.dirname);
	...
	for (const delegation of state.delegations) {
		...
		const id = cwdAlias(workspaceRoot, delegation.cwd);
		if (targets.has(id)) {
			continue;
		}
		targets.set(id, {
...
class WorkspaceDelegationStore {
	...
	async read(): Promise<WorkspaceDelegationState> {
		try {
			...
			return {
				delegations: Array.isArray(parsed.delegations)
					? parsed.delegations.map((item) => item as WorkspaceDelegation)
					: [],
				pendingWakes: Array.isArray(parsed.pendingWakes)
					? parsed.pendingWakes.map((item) => item as WorkspacePendingWake)
					: undefined,
			};
		} catch (error) {
	...
	async write(state: WorkspaceDelegationState): Promise<void> {
...
function workspaceKeyForCwd(workspaceRoot: string, cwd?: string): string | undefined {
	if (!cwd) {
		return undefined;
	}
	const alias = cwdAlias(workspaceRoot, cwd);
	return alias.startsWith("@/") ? alias : undefined;
}

function cwdAlias(workspaceRoot: string, cwd: string): string {
...
function assertInsideWorkspace(workspaceRoot: string, resolved: string, original: string): void {
	const relative = path.relative(workspaceRoot, resolved);
	if (relative === "" || (!relative.startsWith("..") && !path.isAbsolute(relative))) {
		return;
	}
	throw new Error(`Delegation cwd must stay inside workspace root when using @/ or a relative path: ${original}`);
}
