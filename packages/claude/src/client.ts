...
const DEFAULT_SETTING_SOURCES = ["user", "project", "local"] as const satisfies readonly SettingSource[];
...
export class ClaudeCodeClient extends ClaudeCodeEventEmitter {
	...
	constructor(options: ClaudeCodeClientOptions = {}) {
		...
		this.#createQuery = options.createQuery ?? ((input) =>
			query(input) as ClaudeCodeQuery);
	}
	...
	async getSessionMessages(
		sessionId: string,
		options?: Parameters<typeof getSessionMessages>[1],
	) {
	...
	#requestApproval: CanUseTool = async (toolName, input, context) => {
		...
		return await new Promise<PermissionResult>((resolve) => {
			const abortListener = () => {
				...
				resolve({ behavior: "deny", message: "Claude stopped waiting for approval.", interrupt: true });
			};
