...
describe("Actions helpers", () => {
	...
	test("prepareActionsCodexAuth writes base64, raw JSON, API key, and handles missing auth", async () => {
		const base64Root = await tempWorkbench();
		...
		const rawRoot = await tempWorkbench();
		const rawResult = await prepareActionsCodexAuth({
			workbenchRoot: rawRoot,
		...
		const keyRoot = await tempWorkbench();
		const keyResult = await prepareActionsCodexAuth({
			workbenchRoot: keyRoot,
		...
		const missingRoot = await tempWorkbench();
		expect(await prepareActionsCodexAuth({ workbenchRoot: missingRoot, env: {} }))
	...
	test("cleanup removes runtime-only files and preserves durable Actions state and sessions", async () => {
		...
		await writeFile(path.join(codexHome, "memories", "phase2_workbench_diff.md"), "diff");
		await writeFile(path.join(codexHome, "memories", "raw_memories.md"), "keep");
		await writeFile(path.join(codexHome, "workbench", "actions", "state.json"), "{}");

		const result = await cleanupActionsCodexHome({ workbenchRoot: root });

		expect(result.removed).toEqual([
			"auth.json",
			"install_id",
			"memories/.git",
			"memories/phase2_workbench_diff.md",
			"shell_snapshots",
			"state.sqlite",
			"tmp",
		]);
		...
		expect(await readFile(path.join(codexHome, "workbench", "actions", "state.json"), "utf8")).toBe("{}");
	});
});
