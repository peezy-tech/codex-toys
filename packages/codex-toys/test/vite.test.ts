...
describe("codexToysRemote Vite plugin", () => {
	test("serves local bridge endpoints and forwards function calls", async () => {
		const transport = new FakeWorkbenchTransport();
		...
		try {
			...
			const functions = await fetchJson(`${baseUrl}/__codex_toys/api/workbench/functions.list`, {
				method: "POST",
			});
			...
			const described = await fetchJson(`${baseUrl}/__codex_toys/api/workbench/functions.describe`, {
				method: "POST",
			...
			const called = await fetchJson(`${baseUrl}/__codex_toys/api/workbench/functions.call`, {
				method: "POST",
				headers: { "content-type": "application/json" },
				body: JSON.stringify({ name: "snapshot", params: { id: "one" } }),
			});
			expect(called).toEqual({ result: { id: "one", ok: true } });
			const overview = await fetchJson(`${baseUrl}/__codex_toys/api/workbench/overview`, {
				method: "POST",
				headers: { "content-type": "application/json" },
				body: JSON.stringify({}),
			});
			expect(overview).toEqual({ ok: true, workbench: { cwd: "/remote" } });
	...
	test("applies loopback-only CORS on the direct proxy API", async () => {
		const handler = createCodexToysProxyHandler({ transport: new FakeWorkbenchTransport() });
...
class FakeWorkbenchTransport extends CodexEventEmitter implements CodexToyboxTransport {
	...
	async request<T = unknown>(method: string, params?: unknown): Promise<T> {
		...
		if (method === "workbench.overview") {
			return { ok: true, workbench: { cwd: "/remote" } } as T;
		}
